...
public class PagerSlidingTabStrip extends LinearLayout {
    ...
    private int headsContainerMeasuredWidth;
    ...
    private HorScrollView bodyContainer;
    ...
    public PagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
        ...
        defaultTabLayoutParams = new LayoutParams(LayoutParams.WRAP_CONTENT,
    ...
    private void initContainerView(Context context) {
        ...
        //初始化可滚动容器
        bodyContainer = new HorScrollView(context);
        bodyContainer.setLayoutParams(new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT));
        bodyContainer.setFillViewport(true);

        //初始化不可滚动容器
        headsContainer = new LinearLayout(context);
        headsContainer.setOrientation(LinearLayout.HORIZONTAL);
        headsContainer.setLayoutParams(new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT));

        tabsContainer = new LinearLayout(context);
        tabsContainer.setOrientation(LinearLayout.HORIZONTAL);
        tabsContainer.setLayoutParams(new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT));
        bodyContainer.addView(tabsContainer);
        ...
        bodyContainer.setScrollViewListener(new ScrollListener());
    }
    ...
    private void addTab(final int position, View tab) {
        ...
        if (position == 0) {
            ...
            tabsContainer.addView(tab, position - 1, shouldExpand ? expandedTabLayoutParams : defaultTabLayoutParams);
        }
    }

    private void updateTabStyles() {
        boolean isSelected = false;
        for (int i = 0; i < headsContainer.getChildCount(); i++) {
            View v = headsContainer.getChildAt(i);
            if (selectedPosition == i) {
                isSelected = true;
            }
            updateTabStyles(isSelected, v);
        ...
        for (int i = 0; i < tabsContainer.getChildCount(); i++) {
            ...
            if (i + headsContainer.getChildCount() == selectedPosition) {
                isSelected = true;
            }
            updateTabStyles(isSelected, v);
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        headsContainerMeasuredWidth = headsContainer.getMeasuredWidth();
    }

    private void scrollToChild(int position, int offset) {
        if (tabCount == 0 || position - 1 < headsContainer.getChildCount()) {
            bodyContainer.scrollTo(0, 0);
            return;
        }

        int newScrollX = tabsContainer.getChildAt(position - headsContainer.getChildCount()).getLeft();

        if (position > headsContainer.getChildCount() || offset > 0) {
            newScrollX -= scrollOffset;
        }

        if (newScrollX != lastScrollX) {
            lastScrollX = newScrollX;
            bodyContainer.scrollTo(newScrollX, 0);
        }
    }
    ...
    private class PageListener implements OnPageChangeListener {
        ...
        public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
            ...
            if (position > headsContainer.getChildCount() - 1) {
                scrollToChild(position, (int) (positionOffset * tabsContainer.getChildAt(position - headsContainer.getChildCount()).getWidth()));
            }
    ...
    private class ScrollListener implements HorScrollView.ScrollViewListener {
